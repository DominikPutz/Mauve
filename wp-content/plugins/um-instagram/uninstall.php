...
if ( ! empty( $options['uninstall_on_delete'] ) ) {
	...
	global $wpdb;
	$wpdb->query( "DELETE FROM {$wpdb->usermeta} WHERE meta_key='um_instagram_code' OR meta_key='um_instagram_token';" );

	delete_option( 'um_instagram_last_version_upgrade' );
	delete_option( 'um_instagram_version' );
}